<template>
  <v-container>
    <v-layout row wrap v-for="meetup in meetups" :key="meetup.id" class="mb-2">
      <v-flex xs12 sm10 md8 offset-sm1 offset-md2>
        <v-card class="primary">
          <v-container fluid>
            <v-layout row>
              <v-flex xs5 sm4 md3>
                <v-img :src="meetup.imageUrl" height="130px"></v-img>
              </v-flex>
              <v-flex xs7 sm8 md9>
                <v-card-title primary-title class="white--text">
                  <div>
                    <h2 class="display-1">{{meetup.title}}</h2>
                    <p class="subheading">{{meetup.date | date}} - {{meetup.location}}</p>
                    <p class="body-2">{{meetup.description}}</p>
                  </div>
                </v-card-title>
                <v-card-actions>
                  <v-btn flat dark :to="`/meetups/${meetup.id}`">View Meetup
                    <v-icon right>arrow_forward</v-icon>
                  </v-btn>
                </v-card-actions>
              </v-flex>
            </v-layout>
          </v-container>
        </v-card>
      </v-flex>
    </v-layout>
  </v-container>
</template>

<script>
export default {
  computed: {
    meetups() {
      return this.$store.getters.loadMeetups;
    }
  }
};
</script>
